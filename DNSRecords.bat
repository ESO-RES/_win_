...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "      if($line -match '^\s*Address:\s*([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)\s*$'){ [void]$ips.Add($Matches[1]) } " ^
  "      elseif($line -match '^\s*Addresses:\s*(.+)$'){ " ^
  "        foreach($tok in ($Matches[1] -split '\s+')){ if($tok -match '^\d+\.\d+\.\d+\.\d+$'){ [void]$ips.Add($tok) } } " ^
  "      } " ^
  "    } " ^
  "    return $ips.ToArray() " ^
  "  } " ^
  ...
  "    $ips = New-Object System.Collections.Generic.List[string]; " ^
  "    foreach($line in ($out -split \"`r?`n\")){ " ^
  "      if($line -match '^\s*Address:\s*([0-9A-Fa-f:]+)\s*$'){ if($Matches[1] -match ':'){ [void]$ips.Add($Matches[1]) } } " ^
  "      elseif($line -match '^\s*Addresses:\s*(.+)$'){ " ^
  "        foreach($tok in ($Matches[1] -split '\s+')){ if($tok -match ':'){ [void]$ips.Add($tok) } } " ^
  "      } " ^
  "    } " ^
  "    return $ips.ToArray() " ^
  "  } " ^
  ...
  "      # Some nslookup variants emit a single SOA line with all fields; attempt to parse it. " ^
  ...
  "    } " ^
  "    # Fallback: return informative SOA-related lines (stable order) " ^
  "    $vals = New-Object System.Collections.Generic.List[string]; " ^
  "    foreach($line in $lines){ " ^
  "      $t=$line.Trim(); if(-not $t){ continue } " ^
  "      if($t -match 'primary\s+name\s+server\s*=|responsible\s+mail\s+addr\s*=|^serial\s*=|^refresh\s*=|^retry\s*=|^expire\s*=|^minimum\s*='){ $vals.Add($t) | Out-Null } " ^
  "    } " ^
  "    return $vals.ToArray() " ^
  ...
  "      return @((\"{0} {1} {2} {3} {4} {5} {6}\" -f $mname,$rname,$serial,$refresh,$retry,$expire,$minimum)) " ^
  "    } " ^
  "    # Fallback: return any informative SOA lines " ^
  "    $vals = New-Object System.Collections.Generic.List[string]; " ^
  "    foreach($line in ($out -split \"`r?`n\")){ " ^
  "      if($line -match 'primary name server|responsible mail addr|^\s*serial\s*=|^\s*refresh\s*=|^\s*retry\s*=|^\s*expire\s*=|^\s*minimum\s*='){ " ^
  "        $t=$line.Trim(); if($t){ [void]$vals.Add($t) } " ^
  "      } " ^
  "    } " ^
  "    return $vals.ToArray() " ^
  "  } " ^
